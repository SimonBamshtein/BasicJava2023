...
public class Book {
    ...
    private final long isbn; // почему final? - чтобы никто не мог поменять после создания книги
    ...
    //конструктор без автора, на случай, когда нет автора
    ...
    private long checkIsbn(long isbn) {
        if (countDigits(isbn) == ISBN_LENGTH) { // проверка, что 13 цифр в ISBN
            return isbn;
        }
            return -1;
    }
    ...
    private int countDigits(long isbn) {
        ...
        do {
            ...
            isbn /= 10; //делим число на 10, чтобы узнать сколько в нем цифр
        } while(isbn != 0);
        return count;
    }
    ...
    public boolean equals(Object o) {
        ...
        Book book = (Book) o;
        return isbn == book.isbn; // это позволит быстро искать книгу по ее ISBN
    }
